...
- migration audit の `reconstruct` decision は、旧線を延長できないこと、replacement の成果物 identity・読者・scope、systemic finding、replacement root、以後の設計・検証方針を確定するためのものであり、replacement の詳細な章節設計、全 Destination Unit、本文、coverage を完成させる工程ではない。disposition と安全な replacement boundary を独立 review できたら audit を採用し、詳細な網羅的再設計は `reconstruct` 状態の可視化された WIP 制作として続ける。完全な再設計を audit 採用の前提にして、build 可能な replacement source と draft PDF の開始を数日にわたり遅らせない。
- `standards-migration-required` の検出から audit adoption までにも可読状態を失わない。`.system/standards-migration check --operation preview` が許可した current fingerprint 固有の `.workspace/source/reconstruction-preview-<fingerprint-prefix>/` に限り、旧 canonical source と分離した **非 canonical migration-design prototype** を新設し、現在の requirements から再構成した identity、scope、systemic finding、仮 outline、未決定事項と、局所的に根拠を確認できた bounded な実本文を WIP PDF にしてよい。この例外は legacy root の編集・雛形化、新 source の正式統合、旧 review の流用、coverage 主張、audit 完了、replacement 採用、promotion を許可しない。PDF と status は `stale`、`unreviewed WIP`、`noncanonical pre-adoption` を明示する。後の `reconstruct` audit がこの versioned root を replacement root として採用した場合も、各 unit を current requirements、source、依存、coverage で再検証してから継続し、採用しない場合は候補部品として rework または retire する。採用済み `retire` project にはこの preview 例外を適用しない。
...
- build 可能な source がまだないことを、長い source analysis または網羅的再設計のあいだ PDF を出さない理由にしない。成果物 identity、仮 outline、学習目標、dependency、未決定事項を組版できる最小の WIP source spine を最初の長時間作業前に用意し、設計 record 自体を可読 PDF に反映する。安定 snapshot の局所範囲について問い・前提・到達点が仮決定したら、全体設計の完了前でも実際の本文を `provisional` として同じ PDF に載せてよい。未調査範囲を covered と表示せず、後続の inventory、dependency、coverage または identity 判断と齟齬が生じた unit は stale に戻して rewrite または再検証する。
...
- **設計中の provisional authoring**: 全体 inventory または詳細設計の完了前でも、採用済みの安定 snapshot のうち局所的に範囲と根拠を示せる unit は、仮の問い・前提・到達点に基づく実本文として WIP PDF へ載せてよい。これは completion-grade の大量 authoring ではなく、教育設計、用語、密度、依存順を実際の組版で評価する bounded prototype とする。未調査 source との関係、coverage、独立 review は `pending` と表示し、後続判断による invalidation と rewrite を予定された lifecycle として扱う。
