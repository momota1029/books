...
- 暗黙の補題を必要としていないか。必要なら statement と証明を追加するか、正確な外部依存として記録する。
- 代数変形、符号、等号・不等号の向き、零除算、根号・対数等の domain、極限と演算の交換を確認する。
- 全 cases が排反かつ尽くされているか、境界・空集合・零・低次元などの退化ケースが落ちていないか。
- 「同様に」の箇所では、実際の置換を記し、前の議論との差分と追加条件を確認する。
- 外部定理は正確な statement、適用条件、版・番号・位置を原典で確認する。
...
- `label` の一意性、`ref` の存在と参照対象、定理・式・図表番号、文中の参照名を build 結果と照合する。
- project の house macro は定義、引数、意味、scope を確認し、未定義 macro、意味の衝突、表示だけで意味を担う用法を残さない。
- TeX の compile 成功だけで数式の意味が正しいとは判定しない。PDF 上でも欠落 glyph、改行、式の切断、図表との対応を確認する。
