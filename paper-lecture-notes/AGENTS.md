...
- `.workspace/records/bibliography.*`: Paper ID、著者、題名、掲載先、年、DOI/URL、版・改訂、参照日、権利、追加出典。
...
2. 検出結果を `.workspace/records/paper-ingestion-manifest.*` に追記する。各 revision について、少なくとも相対パス、バイトサイズ、更新時刻、可能なら内容 hash、初回検出時刻、処理状態、採用・保留・除外理由を保持する。
3. 同一性をパス、ファイル名、時刻だけで判断しない。可能なら内容 hash で判定し、同名ファイルの内容変更は既存資料の無言の置換ではなく新 revision として登録して、版、主張、式、図表、権利、既存 PDF への影響を評価する。
4. 書き込み・同期・ロック中、一時拡張子、複数回の size/mtime または hash が一致しない、読み取り・parse に失敗するファイルは `pending` として処理しない。安定判定は時間を置いた複数回の stat/hash 一致、またはユーザーの完了合図で行い、長い固定 sleep を前提にしない。
...
7. ユーザーが入力を削除しても manifest の履歴は消さず `missing` として影響を確認する。制作中の追加・更新を見つけても、既存ユーザー入力を移動・削除せず、作業を黙って巻き戻さない。
...
10. `out/` 昇格直前の再走査で、未評価の安定ファイル、変更 revision、または `pending` 入力があれば原則停止して報告する。今回スコープ外とユーザー合意済みの入力だけは、manifest に理由を残して進行できる。
...
- 原論文と外部資料の書誌、版、DOI/URL、ページ、参照日、権利を記録する。直接引用、要約、翻訳に出典を示し、孫引きなら明示する。
- 図表、付録データ、コード、非公開査読資料、個人情報、契約データには別の利用条件があり得る。秘密情報、権利不明素材をコミット・公開しない。

## 数式・図表・PDF 品質

- 記号は初出で定義し、型、次元、定義域、添字範囲、確率変数か実現値かを必要に応じて示す。原論文と記号を変える場合は対応表と理由を残す。
- 式変形の仮定、定理、近似、極限操作を明記し、近似と等号を混同しない。式番号、本文参照、演算子、書体を統一する。
- 図表ごとに出典、元番号、ページ、引用・再描画・改変・新規作成の別、権利を記録する。値、軸、単位、凡例、集計条件、本文参照、白黒・縮小時の可読性を確認する。
...
- 検証記録には実施日、対象 source revision、採用 inbox snapshot、実施項目、結果、残件を残す。Git 差分に原資料、中間物、秘密情報、権利不明素材、無関係な変更がないことを確認する。
